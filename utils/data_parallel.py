    ...
    **scattered** on dim specified (default 0). tuple, list and dict types will
    ...
    def forward(self, *inputs, **kwargs):
        with torch.autograd.profiler.record_function("DataParallel.forward"):
            ...
            import dgl
            graphs = dgl.unbatch(inputs[0][0][0])
            for idx, inp in enumerate(inputs):
                split_graphs = graphs[:inp[0][1].size()[0]]
                del graphs[:inp[0][1].size()[0]]
                split_graphs = dgl.batch(split_graphs).to(inp[0][1].get_device())
                inputs = list(inputs)
                inputs[idx] = list(inputs[idx])
                inputs[idx][0] = list(inputs[idx][0])
                inputs[idx][0][0] = split_graphs
                inputs[idx][0] = tuple(inputs[idx][0])
                inputs[idx] = tuple(inputs[idx])
                inputs = tuple(inputs)
